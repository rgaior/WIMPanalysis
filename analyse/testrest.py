...
#for r in ['run100ks2']:
    ...
    if produce == True:
        ...
        for isplitted in range(splittedsize):
            ...
            com = False
            print  'centerx = ', dsplittedtemp.centerx,' centery = ', dsplittedtemp.centery
            for ifake in range(fakesize):
#            for isplitted in range(splittedsize):
                dfaketemp = dffake.iloc[ifake]
                if ( (dsplittedtemp.RUNID == dfaketemp.RUNID) and (dsplittedtemp.EXTID == dfaketemp.EXTID) and (np.abs(dsplittedtemp.centerx - dfaketemp.centerx) < delX) and np.abs(dsplittedtemp.centery - dfaketemp.centery< delY )) :
#                if (dsplittedtemp.centerx == dfaketemp.centerx) and (dsplittedtemp.centery == dfaketemp.centery):                
...
#                    print 'dtemp.centery = ' , dtemp.centery , ' dsimtemp.centery = ' , dsimtemp.centery
                    print  'fake centerx = ', dfaketemp.centerx,' centery = ', dfaketemp.centery
                    com = True
            if com == True:
                dfcomm = dfcomm.append(dsplittedtemp)                
            else:
                dfrest = dfrest.append(dsplittedtemp)                
...
#        print ' ' , np.min(image[0]), ' ' , np.max(image[0]), ' ' , np.min(image[1]), ' ', np.max(image[1])        
        ...
        plt.ylabel("Y [pixel]")
        plt.colorbar()
        fig.savefig(plotfolder + 'rest_ev_r_'+r+'_run_'+str(runid) + '_ext_'+str(extid) + '_'+str(count)+ '.png')
# plt.show()
