...
for runs,n in zip(allruns,names):
    ...
    expo = 0
    extexpo = {1:0, 2:0, 3:0, 4:0, 6:0, 11:0, 12:0}
    for r in runs:
        expo += utils.getrunexposure(r,extnr)
        rmexpoperext = utils.removedexpofromDCperext(r,dfDC,dclimvalue)
        for key, value in extexpo.iteritems():
            extexpo[key] += -rmexpoperext[key]
    for key, value in extexpo.iteritems():
        extexpo[key] += expo
#    print 'expo tot = ' , expo
#    print extexpo
...
# # plt.show()
    ...
    plt.suptitle(n)
    a_evperext = np.array([])
    a_evperextn = np.array([])
    a_evperextnerr = np.array([])
    for ext in constant.extensionlist:
        dftemp = dfsel.query('EXTID == '+ str(ext))
        a_evperext = np.append(a_evperext,dftemp.shape[0])
        
    for i, expo in zip(range(len(extexpo.values())),extexpo.values()):
        a_evperextn = np.append(a_evperextn,a_evperext[i]/expo)
        a_evperextnerr = np.append(a_evperextnerr,np.sqrt(a_evperext[i])/expo)
    axext.errorbar(constant.extensionlist,a_evperextn,yerr=a_evperextnerr,fmt='o')
