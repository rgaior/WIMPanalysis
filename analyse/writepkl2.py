...
for f in files:
    ...
    if first == True:
        ...
        if code == 'a':
            dfall = utils.readsimtree(f,exposure,type)
        elif code == 'p':
            dfall = utils.readsimtreeP(f,type)
...
#############################
        ...
        # simcluster trees
        dfsimall = utils.readsimclustertree(f,exposure,type)
...
#        dfsim.to_pickle(outfolder  + '/simc_' + outname[1:])
        ...
        #        df = utils.readsimtreeori(f,type)
        if code == 'a':
            df = utils.readsimtree(f,exposure,type)
        elif code == 'p':
            df = utils.readsimtreeP(f,type)
...
if code == 'a':
    dfgood = dfall[(dfall.touchmask == 0) & (dfall.centery < 44 )]       
