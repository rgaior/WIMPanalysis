...
#runs = ['run100ks1','run100ks2','run100ks3','run30ks1','run30ks2','run30ks3']
#runs = ['run100ks1','run100ks2','run100ks3','run30ks1','run30ks2']
runs = ['run30ks4']
exposure = 0
for r in runs:
    exposure+=constant.runinfo[r][0]*constant.runinfo[r][1]
print 'exposure = ' , exposure

lim_eventrate = 1e-2/(0.6*24*3600)
...
# perform cuts
#dfsel = dfall.query(constant.basecuts + ' & ' + fidcut)
#ad_hoccut = 'RUNID > 3331 & RUNID < 3337' 
#ad_hoccut = 'RUNID > 3203 & RUNID < 3216' 
#ad_hoccut = 'RUNID > 3250 & RUNID < 3268' 
#ad_hoccut = 'RUNID > 3331 & RUNID < 3337' 
ad_hoccut = 'RUNID > 3453' 
dfsel = dfall.query(constant.basecuts + ' & ' + fidcut + ' & ' + ad_hoccut)
#dfsel = dfall.query(constant.basecuts + ' & '+ ad_hoccut)

#dfsel = dfall.query(constant.basecuts)
listofid = pd.unique(dfsel.RUNID)
exposure = len(listofid)*3e4

limevent = lim_eventrate*exposure*42e-3
print 'limevent = ', limevent
firstbin = -50
step = 0.2
lastbin = 0
...
print '----------------------------------'
print 'average DC ADU = ', dfDC[['ADUDC']].mean()*100
print '----------------------------------'
