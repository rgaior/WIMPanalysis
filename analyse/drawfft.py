...
f = R.TFile(file)
im = f.Get("image")
image = utils.getimage(im)
#print image[2]
if nelec == 0:
    f, axarr = plt.subplots()
    f.suptitle('cluster on RUNID: '+ str(runid) + ' ext: '+ str(extid),fontsize=20)
    fft = np.fft.fft2(image[2])
    absfft = np.abs(fft)
#    plt.imshow(image[2],origin='lower',extent=[np.min(image[0]),np.max(image[0])+1,np.min(image[1]),np.max(image[1])+1],aspect='auto')
#    plt.imshow(absfft,origin='lower',extent=[np.min(image[0]),np.max(image[0])+1,np.min(image[1]),np.max(image[1])+1],aspect='auto')
    plt.imshow(absfft,origin='lower', aspect='auto')
